...
Present 3 options:
- **(A) News Hunting** — Breaking news. Prioritizes freshest trends, output focuses on Content Drafts.
- **(B) Community Engagement** — High-value replies. Prioritizes Reply Strategy, scans trending threads.
- **(C) Evergreen Only** — No trend dependency. User picks topic from niche_keywords for educational content.

If **(C)** → skip scanning and scoring, go directly to content generation.
...
### 🐦 Source Priority: X Trending → Twitter → Reddit → News

Scan in this order — X Trending discovers **unknown trends**, keyword search finds **known niche updates**:

#### Source 0 (Discovery): X Trending Topics

Scan what's currently trending on X to discover trends **outside** your keyword list:

```
STEP A: Fetch trending list
  Tavily:   extract.mjs "https://trends24.in/united-states/"
  Fallback: web_search "twitter trending topics today crypto"

STEP B: Filter vs niche_keywords
  → Compare each trending topic against niche_keywords
  → Keep topics that match ≥ 1 keyword (exact or semantic)
  → Example: Trending "#OnchainAI" matches niche "On-chain AI" ✅
  → Discard unrelated trends (e.g., "#Grammy2026" = no match)

STEP C: Inject matched topics into keyword search
  → Add matched trending topics to the search queue
  → They get scanned through Sources 1-3 below like any other keyword
```

> ⚠️ **trends24.in is not an official API** — if inaccessible, skip Source 0 silently and continue with keyword-based scanning. This step is additive, not blocking.

#### Source 1 (Fastest): Twitter/X keyword search
...
To read full articles or do deep research:

| Action | Tavily | Fallback |
|--------|--------|----------|
| Read article | `extract.mjs "{url}"` | `web_fetch` with URL |
| Deep research | `search.mjs "{keyword}" --deep --topic news --days 1` | Multiple `web_search` queries with different phrasings |
...
Built-in sensitive keywords: politicians, political parties, war/conflict terms, strong negatives (arrested, died, scam, fraud, bankrupt). See `references/sensitive_keywords.md` for full list.

If matched: flag `[SENSITIVE]` → ask user "🔴 Sensitive Trend: '[name]'. Proceed? (Yes/No)" → Yes = generate with `[USE WITH CAUTION]`, No = skip.

Also check user's `blacklist` — any match = auto-discard silently.
...
START
  ...
  ▼
  SCAN:
    Source 0: X Trending (discover unknown trends) → filter vs niche_keywords
    Source 1-3: Twitter/X → Reddit → News [keyword search, freshness enforced]
  │
