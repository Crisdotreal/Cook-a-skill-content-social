...
| **Fallback** | `web_search` with each keyword + "news today" or "breaking" |
...
| Engine | Command |
|--------|---------|
| **Tavily** | `search.mjs "{keyword}" --deep --topic news` |
| **Fallback** | Run multiple `web_search` queries with different phrasings |
...
| **Fallback** | `web_search` with `site:x.com` or `site:twitter.com` |
...
| Engine | Fallback |
|--------|----------|
| **Tavily** | `search.mjs "{keyword} site:reddit.com" -n 10` |
| **Fallback** | `web_search` with `site:reddit.com` |
...
Aggregate all results and pass to **Security Check** before scoring.
...
**Why this matters:** A reply that repeats what 50 people already said gets ignored. A reply that adds a *new* perspective gets engagement. Thread context is the difference between "noise" and "signal."

**Fallback when thread is inaccessible:** If the thread URL cannot be fetched (private account, deleted, rate-limited), note this in the output and write the reply based on the trend summary only. Tag the reply as `[NO THREAD CONTEXT]`.
...
**Good — Tech category** (AI Agents, On-chain AI):
```
...
**Good — Degen Plays category** (Memecoins, Solana, Base):
```
Hook: "Another day, another dog coin doing a 50x on Solana while your blue chips bleed." 🐕📈
Body: The memecoin meta is undefeated. You can cope about fundamentals all day — meanwhile some degen just turned $200 into a used Lambo.
Base is cooking too. The on-chain activity is giving early Solana vibes.
CTA: "What's your degen play this week? Drop it 👇"
```

**Bad** (off-brand — press release tone):
```
"Exciting developments in the Ethereum ecosystem regarding AI agents. According to recent reports, Ethereum has seen significant growth in AI agent deployments. What are your thoughts on this development? Share below."
```

Good copy uses CT slang, humor, line breaks for punchiness — not press releases. Check `style_guide` for brand-specific rules. When `brand_voice` is "Professional & Informative", lead with data and cite sources explicitly.
...
Unless a dedicated Twitter/Reddit MCP with Enterprise API access is connected, this skill relies on public web search indexing (Tavily or web_search). This may result in a **15–60 minute latency** compared to real-time social feeds. It is a "Sniper", not a high-frequency trading bot.

Additionally, web search results are biased toward **English-language major outlets** — alpha from Chinese CT, Korean Telegram groups, or Japanese crypto communities may be missed.

> **Mitigation:** Connect Twitter/X and Reddit MCP connectors for fresher data. For non-English alpha, add localized keywords to `niche_keywords` (e.g., "仮想通貨 AI", "크립토 에이전트").

### 2. Heuristic Scoring (Not ML)

The relevance scoring (10-point scale) is **deterministic and rule-based**. It strictly follows the provided rubric and does not use Machine Learning to "learn" user preferences over time.

The engagement threshold is fixed at >100 substantive replies — this may be too high for small niches or too low for mainstream trending topics. The Tone Match sub-criterion also involves AI judgment, which may vary slightly between sessions.

> **Mitigation:** Treat scores as directional guidance, not absolute truth. Review BACKUP trends (5–6 pts) manually — they may contain hidden gems the rubric undervalues. Future versions may introduce adaptive thresholds based on niche size.

### 3. Verification Scope (Garbage In, Garbage Out)

The "Zero Hallucination" goal ensures every claim has a source link. However, the skill **cannot verify the truthfulness of the sources themselves**. If misinformation is widely reported by multiple outlets, the skill may reflect that consensus.

Source quality is assessed only by count (single vs. multi-source), not by credibility. Three blog spam articles still pass the multi-source check. The scam URL filter is pattern-based and may miss novel scam patterns.

> **Mitigation:** Always verify `[UNVERIFIED]` trends before publishing. For high-stakes content, cross-check source credibility manually. Report new scam URL patterns so the filter can be updated.

### 4. Text-Only Execution

The skill generates **strategic text drafts only**. It does not:
- Auto-post to any platform (Twitter, Reddit, etc.)
- Generate images, memes, or visual assets
- Edit or produce video content
- Schedule or queue posts

> **Mitigation:** Use the output as input for your existing publishing workflow. Pair with image generation tools (Midjourney, DALL·E) for visual content. Copy-paste drafts into your scheduling tool (Typefully, Buffer, etc.).
